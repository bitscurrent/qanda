...
export const signup = async (req, res) => {
    ...
    try {
        ...
        const newStudent = new Student({ name, username, email, password: hashedPassword });
...
export const login= async (req, res) => {
    ...
    try {
      ...
      res.json({ success: true, token, name: student.name, email: student.email, });
      } catch (error) {
  ...
  export const askquestion =  async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({ success: true, message: 'Question ID saved successfully', student });
    } catch(error) {
...
export const askquestionview = async (req, res) => {
    ...
    try {
      ...
      const students = await Student.find({ email: studentId }, { askedQuestionsId: 1 });
      const askedQuestionsId = students.map(student => student.askedQuestionsId).flat(); // Extract askedQuestionsId arrays from students and flatten them
...
export const rtebystudent = async (req, res) => {
    try {
      const { question, studentId, date, solve,  askedBy,
      askedOn, } = req.body;
      const assigned = false;
  
      // Create a new Asset object with the provided data
      const newHtmlContent = new Asset({ question: question, studentId: studentId, date: date, solve: solve,  assigned, askedOn, askedBy});
...
export const getassetbyid=async (req, res) => {
    ...
    try {
      ...
      const {question, answer, date, solvedOn,feedback,likes,dislikes,comments,} = asset;
      // Respond with the extracted data
      res.json({question, answer, date, solvedOn, feedback,likes,dislikes,comments,});
    } catch (error) {
